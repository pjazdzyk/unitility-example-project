...
public final class DryAirPropertiesEquations {
    ...
    Density density(Temperature temp, Pressure press) {
        double tk = temp.getInKelvins();
        double pa = press.getInPascals();
    ...
    SpecificEnthalpy specificEnthalpy(Temperature temp) {
        ...
        double cp = specHeat.getInKiloJoulesPerKiloGramKelvin();
        double ta = temp.getInCelsius();
        double i = cp * ta;
    ...
    KinematicViscosity kinematicViscosity(Temperature temp, Density density) {
        double densityVal = density.getInKilogramsPerCubicMeters();
        double dynVis = dynamicViscosity(temp).getInKiloGramPerMeterSecond();
        double kinVis = dynVis / densityVal;
